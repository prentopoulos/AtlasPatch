...
| 4 | `add-conductor-distribution` | In progress | Wire the four logical agents as A2A peers (Google ADK + A2A), add the optional BigQuery telemetry backend, and the GUI Level 2 message-flow view. A2A earns its weight here as watchable choreography (design D8); the core already runs without it. |
