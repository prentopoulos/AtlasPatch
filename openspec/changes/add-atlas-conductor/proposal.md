...
- Introduce four A2A agents — **planner, execution worker, validator, recovery** — coordinated by an in-process scheduler.
...
- Add an **optional telemetry sink** — local (jsonl/sqlite) by default, BigQuery optional — storing operational metadata only, **PHI-free by construction** (pseudonymized slide stems + a deterministic write-time gate rejecting HIPAA Safe-Harbor identifiers), and doubling as an **append-only labeled dataset of recovery outcomes** for later classifier learning.
- Add **governance-by-construction**: a human-in-the-loop confirmation gate on irreversible/expensive recovery actions (`force_reprocess`, `block_job`, `quarantine_item`), a no-PHI/no-pixel **egress assertion**, an **audited append-only trail**, and a **Model Card** — keeping the layer verifiably operational-only and out of Software-as-a-Medical-Device scope.
...
### New Capabilities
- `orchestration-run`: top-level run lifecycle — YAML job config intake, terminal summary report, and the hard boundary invariants (upstream untouched, CLI-only integration, metadata-only telemetry, structural-not-clinical validation).
...
- `run-telemetry`: an append-only operational-metadata sink with local and BigQuery backends, whose record types make the metadata-only invariant impossible to violate, and which captures labeled `(signature → classification → action → resolved?)` recovery outcomes.
- `governance-compliance`: the by-construction guardrails — deterministic operational-only core (no clinical reasoning), PHI-free telemetry, no-PHI/no-pixel egress, HITL gate on irreversible actions, audited append-only trail, and a Model Card — each with a CI proof so the safeguards are demonstrable, not asserted.
...
- **Dependencies**: new optional extra `atlas-patch[orchestrator]` (Google ADK, A2A, `google-cloud-bigquery`, `pyyaml`); core install unaffected.
