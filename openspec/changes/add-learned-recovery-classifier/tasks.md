...
- [x] 5.1 Add `classifier/dataset.py`: read `slide_stage_outcomes` via `TelemetrySink.read_slide_stage_outcomes()`, reconstruct `(features, label)` per row with label precedence `injected_label` → recorded `classification` → skip; return arrays plus the class ordering.
- [x] 5.2 Add `classifier/train.py` (`train_model(dataset, seed) -> LinearModel`) and `classifier/evaluate.py` (`evaluate(classifier, dataset) -> {accuracy, per_class_precision_recall, safety_metric}`) where `safety_metric` = fraction of should-block rows the composed classifier would retry.
- [x] 5.3 Add tests over a synthetic recovery dataset generated by the fake adapter's injections: accuracy ≥ threshold against `injected_label`, and `safety_metric == 0` for the composed `LearnedClassifier`.
